...
const CmcCoinChartTS = require("../charts/charts.model");
...
const CoinsDominancecharts = require("./models/cmc-dominance-chart");
...
exports.chartbyId = async ({ id, interval }) => {
  ...
  try {
    ...
    const data = await CmcCoinChartTS.find(query)
      .sort({ timestamp: 1 })
      .select("timestamp market_cap volume -_id")
      .lean();
...
exports.getTopstats = async ({ id }, result = {}) => {
  try {
    ...
    const dominancecharts = await CoinsDominancecharts.findOne({
      _id: new ObjectId("68f63f5b02765f1573f0de1f"),
    });
    result.data = {
      coinStats,
      btcSentiment,
